...
namespace LINQProblems
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            List<FileInfo> files = GetAllPNG(@"C:\Users\Ryan\Pictures\"); //NOTE: READ SUMMARY COMMENT ABOVE METHOD
            GetFibonacciNumbers();
            SumPrimes();
        }
        ...
        public static List<FileInfo> GetAllPNG(string path)
        {
            ...
            //files = new DirectoryInfo(path).EnumerateDirectories("*", SearchOption.AllDirectories)
            //    .Select(directory => directory.EnumerateFiles("*.png"))
            //    .Aggregate((total, current) => total.Concat(current)).Concat(new DirectoryInfo(path).EnumerateFiles("*.png")).OrderBy(t => t.Name).ToList();
            return files;
        }
        ...
        public static List<int> GetFibonacciNumbers()
        {
            ...
            return Enumerable.Range(1, 39).Where(x => (x - 1) % 5 == 0).Select(x =>
                {
                    Func<int, int> func = null;
                    func = (t) => (t == 1 || t == 2) ? 1 : func(t - 1) + func(t - 2);
                    return func(x);
                }).ToList();
        }
        ...
        {
            return Enumerable.Range(2, 1000).Where(t => !Enumerable.Range(2, t / 2 + 1).Any(x => t != x && t % x == 0)).Sum();
